...
import { InvoiceList, order } from '../questoes';
...
export class AppEditInvoiceComponent {
  ...
  fillAddControls(): void {
    this.addForm.setControl('item', this.setItem(this.invoice));
  }
  ...
  btnRemoveClick(i: number): void {
    let totalCostOfItem =
      this.addForm.get('item')?.value[i].itemCost * this.addForm.get('item')?.value[i].itemSold;
